...
exports.updateLayerPropertiesWithCounts = updateLayerPropertiesWithCounts;
...
function updateLayerPropertiesWithCounts(layer, counts, matchingKey) {
    if (!layer.keyLookup || !layer.valLookup) buildKeyValueLookups(layer);
    const countKey = layer.keyLookup[matchingKey];

    for (var i = 0; i < layer.features.length; i++) {
        var feature = layer.features[i];
        if (feature.tags) {
            for (var j = 0; j < feature.tags.length; j += 2) {
                if (countKey === feature.tags[j]) {
                    let val = layer.values[feature.tags[j + 1]];
                    let count = counts[val] || 0;
                    addKey('count', layer.keyLookup, layer.keys, feature.tags);
                    addValue(count, layer.valLookup, layer.values, feature.tags);
                }
            }
            // console.log(layer.values);
            // console.log(matchingKey);
            // console.log(layer.keyLookup);
            // console.log(valKey);
            // console.log(layer.valLookup);
            // let val = layer.values[feature.tags[1]];
            // let count = counts[val] || 0;
            // addKey('count', layer.keyLookup, layer.keys, feature.tags);
            // addValue(count, layer.valLookup, layer.values, feature.tags);
        }
